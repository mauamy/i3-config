...
chosen="$(rofi -sep "|" -dmenu -i -p 'display' -theme ~/.cache/wal/colors-rofi-dark.rasi -location 0 -xoffset -14 -yoffset -52 -width 10 -hide-scrollbar -line-padding 4 -padding 20 -lines 4 <<< "laptop|dual|external|clone")"
...
if [ `xrandr | grep "^DP-1" | grep -c ' connected '` -eq 1 ]; then
        EXTERNAL_OUTPUT=$("xrandr | grep "^DP-1" | grep " connected" | cut -d' ' -f1")
fi
...
case "$chosen" in
    external) xrandr --output $INTERNAL_OUTPUT --off --output $EXTERNAL_OUTPUT --auto --primary; systemd-inhibit --what=handle-lid-switch sleep 1d;;
